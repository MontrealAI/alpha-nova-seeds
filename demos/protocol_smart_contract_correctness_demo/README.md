...
The winner is selected with a deterministic **lexicographic rank key** (not a single composite formula):

- accepted usefulness points (higher is better)
- time to first accepted output (lower is better)
- repair/rework ratio (lower is better)
- evidence completeness (higher is better)
- unsupported claim rate (lower is better)
- severity inflation count (lower is better)
- packageable artifact quality (higher is better)

This ordering matches `src/assay.py` `run_mandate_1_competition` and is replay-safe across runs.

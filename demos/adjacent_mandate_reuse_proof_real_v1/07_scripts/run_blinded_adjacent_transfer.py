...
def cmd_build_kits() -> int:
    ...
    for filename in KIT_FILENAMES:
        ...
        blue_integrity = _sha256_file(blue)
        gold_integrity = _sha256_file(gold)
        parity[filename] = {
            ...
            "blue_integrity_sha256": blue_integrity,
            "gold_integrity_sha256": gold_integrity,
...
def cmd_lock_score() -> int:
    ...
    for lane in ["lane_blue_packet_public", "lane_gold_packet_public"]:
        ...
        for name in required_packet_files:
            p = lane_dir / name
            if not p.exists():
                packet_issues.append(f"{lane}/{name} missing")
                continue
            text = p.read_text(encoding="utf-8")
            if "AWAITING_LANE_OPERATOR_OUTPUT" in text:
                packet_issues.append(f"{lane}/{name} still template")
    if packet_issues:
        _write_json(PUBLIC_DIR / "summary_metrics.json", {
            "status": "SCORE_LOCK_PENDING_HUMAN_INPUT",
            "timestamp": _now(),
            "note": "Score lock requires completed blinded reviewer packets.",
    ...
    if any((not p.exists()) or _is_template_like_csv(p) for p in required):
        _write_json(PUBLIC_DIR / "summary_metrics.json", {
            "status": "SCORE_LOCK_PENDING_HUMAN_INPUT",
            "timestamp": _now(),
            "note": "Score lock requires real filled CSV inputs.",
            "demonstrated": [
                "run-ready blinded adjacent-transfer harness",
                "package freeze and kit-building pipeline",
            ],
...
def cmd_validate_readiness() -> int:
    ...
    if prereg_path.exists():
        ...
        for rel, expected_hash in expected_scope_hashes.items():
            current_hash = current_scope_hashes.get(rel)
            if current_hash != expected_hash:
                reasons.append(
                    f"scope hash mismatch: {rel} expected={expected_hash} current={current_hash}"
                )
    ...
    manifest_path = PUBLIC_DIR / "kit_manifest_public.json"
    if manifest_path.exists():
        kit_manifest = _read_json(manifest_path)
        parity_checks = kit_manifest.get("parity_checks", {}) if isinstance(kit_manifest, dict) else {}
        for filename in KIT_FILENAMES:
            meta = parity_checks.get(filename, {})
            blue = KITS_DIR / "kit_blue" / filename
            gold = KITS_DIR / "kit_gold" / filename
            expected_blue = meta.get("blue_integrity_sha256")
            expected_gold = meta.get("gold_integrity_sha256")
            if not expected_blue or not expected_gold:
                reasons.append(f"kit manifest missing integrity hash for {filename}")
                continue
            blue_hash = _sha256_file(blue)
            gold_hash = _sha256_file(gold)
            if blue_hash != expected_blue:
                reasons.append(f"kit integrity mismatch for kit_blue/{filename}")
            if gold_hash != expected_gold:
                reasons.append(f"kit integrity mismatch for kit_gold/{filename}")

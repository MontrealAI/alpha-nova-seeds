...
- `assemble_reveal_packet.py` now rejects scaffold placeholder `private_commitment_hashes.txt` content (and malformed digest rows) so reveal receipts cannot be generated without real commitment hashes.
- `assemble_reveal_packet.py` now marks `revealed_after_score_lock` as `true` in emitted public receipt rows when reveal assembly succeeds, avoiding internal score-lock/reveal-state contradictions.

...
- Split MARK and Architect schema surfaces between legacy actionable contracts (`mark_selection_report.schema.json`, `architect_recommendation.schema.json`) and ascension-runtime aggregate contracts (`mark_selection_bundle.schema.json`, `architect_next_loop.schema.json`) so selection and next-step fields remain mandatory where expected.

...
describe("NovaSeedWorkflowAdapterV25 fail-closed routing", function () {
  it("only allows owner workflow actions and checks seed-state gate", async function () {
    ...
    await expect(workflow.connect(outsider).setMARK(await mark.getAddress())).to.be.revertedWith("OwnableUnauthorizedAccount");
    ...
    await expect(workflow.connect(outsider).createAssay(seedId, assaySpecHash, 100n)).to.be.revertedWith("OwnableUnauthorizedAccount");
    ...
    await expect(workflow.connect(outsider).finalizeAssay(seedId, 1n)).to.be.revertedWith("OwnableUnauthorizedAccount");
